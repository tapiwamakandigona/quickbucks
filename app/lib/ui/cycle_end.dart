...
class CycleSettingsScreen extends StatelessWidget {
      ...
      body: ListView(
        ...
        children: [
          if (cycle != null) ...[
            ...
            Card(
              child: ListTile(
                leading: const Icon(Icons.celebration, size: 32, color: kGold),
            ...
            child: Column(
              children: [
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Replaces what is on this phone with the file',
                  ),
  ...
  Future<void> _editDate(BuildContext context, {required bool isStart}) async {
    ...
    try {
      ...
      if (context.mounted) {
        showNote(context, 'Could not update: ${friendlyError(e)}', error: true);
      }
    }
  }
  ...
  Future<void> _endContributions(BuildContext context) async {
    ...
    final ok = await confirmAction(
      ...
      message:
          'No more Saturday payments will be collected. Members keep '
    ...
    try {
      ...
      if (context.mounted) {
        showNote(context, 'Could not end: ${friendlyError(e)}', error: true);
      }
    }
  }
  ...
  Future<void> _shareOut(BuildContext context) async {
    ...
    final ok = await confirmAction(
      ...
      message:
          'Cash in the box: ${money(t.cashOnHandCents)}\n'
    ...
    try {
      ...
      if (context.mounted) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => ShareOutSheetScreen(cycle: cycle)),
        );
      }
    } catch (e) {
      if (context.mounted) {
        showNote(
          context,
          'Could not share out: ${friendlyError(e)}',
          error: true,
        );
      }
    }
...
Future<void> _backup(BuildContext context) async {
  ...
  try {
    ...
    final file = File('${dir.path}/QuickBucks backup ${iso(today())}.json');
    ...
    if (context.mounted) {
      showNote(
        context,
        'Could not make the backup: ${friendlyError(e)}',
        error: true,
      );
    }
  }
...
Future<void> _restore(BuildContext context) async {
  ...
  final ok = await confirmAction(
    ...
    message:
        'Everything currently in the app will be REPLACED by the '
...
Future<void> _restoreAuto(BuildContext context) async {
  ...
  if (files.isEmpty) {
    showNote(
      context,
      'No automatic backups yet — use the app once and one '
      'will be saved.',
    );
    return;
  }
  ...
  final ok = await confirmAction(
    ...
    message:
        'Everything currently in the app will be REPLACED by that '
...
String _shareOutText(
  Cycle cycle,
  ShareOut shareOut,
  List<ShareOutLine> lines,
  ...
  for (final l in lines) {
    b.write(
      '${nameOf(l.memberId)} (×${l.multiplier}): '
      '${money(l.shareCents)}',
    );
      ...
      body: FutureBuilder(
        ...
        builder: (context, snap) {
          ...
          if (so == null) {
            return const Center(
              child: Text(
                'No share-out recorded for this cycle.',
                  ...
                  child: ListTile(
                    ...
                    trailing: Text(
                      ...
                      style: TextStyle(
                        ...
                        color: l.payoutCents < 0 ? kDanger : Colors.black87,
                      ),
                    ),
                  ),
                ),
              Padding(
                ...
                child: FilledButton.icon(
                  onPressed: () => Share.share(
                    _shareOutText(cycle, shareOut, lines, nameOf),
                  ),
          ...
          : (v) async {
              ...
              if (!ok) {
                messenger.showSnackBar(
                  const SnackBar(
                    content: Text(
                      'The phone did not allow notifications. '
